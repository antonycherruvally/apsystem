...
//
		 $sql = "SELECT * FROM employees WHERE id = '$status'";
		...
		if($query->num_rows > 0){
          ...
          //   echo $id;
			//$date_now = date('Y-m-d');
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'this employee is already added';
				}
				else{
					//updates
					...
					//
					$sql = "INSERT INTO attendance (employee_id, date, time_in, status) VALUES ('$status', '$date_now', NOW(), '$day')";
					
					if($conn->query($sql)){
						$output['message'] = 'Added: '.$row['firstname'].' '.$row['lastname'];
					}
					else{
						$output['error'] = true;
