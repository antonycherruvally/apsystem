	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
		   ...
		   $id = $row['id'];
			...
			if($status == 'in'){
				$sql = "SELECT * FROM attendance WHERE id = '$id' AND date = '$date_now' AND time_in IS NOT NULL";
				$query = $conn->query($sql);
				if($query->num_rows > 0){
					$output['error'] = true;
					...
					//
					$sql = "INSERT INTO attendance (employee_id, date, time_in, status) VALUES ('$id', '$date_now', NOW(), '$day')";
					if($conn->query($sql)){
